...
<section class="mx-20 py-4 h-screen">
    <TabGroup as="section" class="grid gap-6 grid-cols-1 lg:grid-cols-12 mt-8" vertical>
        <TabList as="div" class="col-span-3 flex flex-col text-sm space-y-2 focus:outline-0">
          <template v-for="item in pages">
            <span v-if="item.type === 'header'" class="font-bold text-slate-400 pt-1">{{ item.name }}</span>
            <Tab :disabled="item.disabled" v-slot="{ selected }" v-else-if="item.type === 'menu'" class="focus:outline-0 w-7/12">
              <button :class="[
        ...
        <TabPanels class="col-span-9 focus:outline-0 h-screen">
